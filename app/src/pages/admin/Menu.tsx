...
export function AdminMenu() {
  ...
  const addImageUrl = () => {
    ...
    if (!next || images.length >= 7) return;
  ...
  return (
    <GlassCard title="Menu Manager" subtitle="Dish name, description, category, price and up to 7 images.">
      <div className="grid gap-4 md:grid-cols-2">
        <GlassInput label="Dish Name" placeholder="Paneer Lababdar" value={name} onChange={(e) => setName(e.target.value)} />
        <GlassInput label="Price" type="number" placeholder="420" value={price} onChange={(e) => setPrice(e.target.value)} />
        <GlassInput label="Description" placeholder="Dish details" value={description} onChange={(e) => setDescription(e.target.value)} />
          <label className="flex flex-col gap-2 text-sm text-white/70">
          Category
          <select className="glass glass-hover rounded-full bg-white/75 px-4 py-2 text-black" value={categoryId} onChange={(e) => setCategoryId(e.target.value)}>
            {categories.map((cat) => (
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto]">
        <GlassInput label="Image URL" placeholder="https://..." value={imageInput} onChange={(e) => setImageInput(e.target.value)} />
        <div className="flex items-end">
          <GlassButton variant="secondary" onClick={addImageUrl} disabled={images.length >= 7}>Add URL</GlassButton>
        </div>
        <div className="flex items-end">
          <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={onUploadImages} />
          <GlassButton variant="secondary" onClick={() => fileInputRef.current?.click()} disabled={images.length >= 7}>Upload Image</GlassButton>
        </div>
      </div>
        ...
        {images.map((img, idx) => (
          <div key={`${img}-${idx}`} className="glass rounded-2xl p-2">
            ...
            <div className="mt-2 flex items-center justify-between text-xs text-white/70">
              ...
              <button className="text-red-200 hover:text-red-100" onClick={() => removeImageAt(idx)}>Remove</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-4 flex gap-2">
        ...
        {editingId && <GlassButton variant="ghost" onClick={resetDishForm}>Cancel</GlassButton>}
      </div>
            ...
            <div key={item.id} className="rounded-2xl border border-white/10 p-3">
              <div className="grid gap-3 md:grid-cols-[120px_1fr_auto] md:items-center">
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs text-white/60">Category: {categories.find((c) => c.id === item.categoryId)?.name || item.categoryId}</div>
                  <div className="mt-1 text-xs text-white/60">{imgs.length} image(s)</div>
                </div>
                <div className="flex gap-2">
                  <GlassButton variant="secondary" onClick={() => editDish(item)}>Edit</GlassButton>
                  <GlassButton variant="ghost" onClick={() => removeDish(item.id)}>Remove</GlassButton>
                </div>
              </div>
            </div>
          );
